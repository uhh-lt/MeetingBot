  ...
  export default {
    ...
    methods: {
      ...
      handleNewKeywords(keywords) {
        ...
        let lowKeywords = keywords.flatMap(value => value.word.split(' ')).map(value => value.toLowerCase());
      ...
      computeAllKeywords() {
        // shorten keyword map
        let maximum = Math.min(this.keywordMap.size, this.maxKeywords);
        let shortMap = new Map([...this.keywordMap.entries()].slice(0, maximum));
        // convert to array
        let result = [];
