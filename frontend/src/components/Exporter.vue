  ...
  export default {
    ...
    methods: {
      ...
      getEditedUtteranceText(agendaID, utteranceID) {
        let elementID = "agenda-"+agendaID+'-utterance-'+utteranceID;
      ...
      visualizeUtterance(utterance) {
        ...
        for(let i = 0; i < tokens.length; i++) {
          let token = tokens[i];
          let confidence = utterance.confidences[i];
      ...
      createPDF() {
        ...
        for(let i = 0; i < this.editorAgendaPoints + 1; i++) {
          let agendaTitle = i < this.editorAgendaPoints ? this.editorAgendaTitles[i] : 'Sonstiges';
          html += '<h2>'+agendaTitle+'</h2>';

          let start = true;
          let lastSpeaker = -1337;
          let utterances = this.editorUtterances[i];
          let utterance;
        ...
        let filename = 'Meeting_'+this.editorAgendaDate.replace(/\./g, "-")+'.pdf';
        pdf.fromHTML(
